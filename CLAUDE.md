...
- `js/open5e.js` loads with data.js before app.js. `js/bestiary.js` is ~1 MB and
  is injected on demand the first time the Bestiary tab is opened; it calls
  `onBestiaryLoaded()` when ready.
...
- Monstrous Menagerie (the Bestiary) is **OGL 1.0a only**. `OGL.txt` carries the
